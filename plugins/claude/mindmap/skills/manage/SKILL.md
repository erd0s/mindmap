...
Automatic context is a bounded partial view. It prioritizes a bounded set of semantic warning codes and exact concept IDs, reports omitted warnings/concepts and previews long prose; missing rows are not deleted concepts, and an omitted parent row does not make a child a root. Retrieve the full snapshot before editing an omitted concept, checking relationships, restoring a deleted branch, or giving a complete status playback. Do not overwrite full summaries or resumes with their previews. The injected `schema` command gives the complete record contract and numeric limits from the validator.
